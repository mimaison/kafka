...
class KafkaConfig private(doLog: Boolean, val props: java.util.Map[_, _], dynamicConfigOverride: Option[DynamicBrokerConfig])
  ...
  val cordonedLogDirs: Set[String] = Csv.parseCsvList(getString(ServerLogConfigs.CORDONED_LOG_DIRS)).asScala.toSet
  ...
  private def validateValues(): Unit = {
    ...
    require(cordonedLogDirs.forall(cordonedLogDir => logDirs.contains(cordonedLogDir)), "")//TODO
