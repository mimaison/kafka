...
import org.apache.kafka.common.requests.{FindCoordinatorsRequest, FindCoordinatorsResponse}
...
class ConsumerBounceTest extends AbstractConsumerTest with Logging {
  ...
  private def findCoordinator(group: String): Int = {
    val request = new FindCoordinatorsRequest.Builder(new FindCoordinatorsRequestData()
      .setKeyType(FindCoordinatorsRequest.CoordinatorType.GROUP.id)
      .setKey(group)).build()
    var nodeId = -1
    TestUtils.waitUntilTrue(() => {
      val response = connectAndReceive[FindCoordinatorsResponse](request)
