...
public class MMAssignorTest {
    ...
    public void testAssignReplicasToBrokers() {
        ...
        Map<Integer, List<Integer>> assignment = assignor.assignReplicasToBrokers("topic", 3, 3, 0, cluster, principal);
        System.out.println(assignment);
        List<Integer> replicas = assignment.get(0);
        assertNotNull(replicas);
        Set<Integer> replicaSet = new HashSet<>(replicas);
    ...
    public void testAddPartition() {
        ...
        Cluster cluster = buildCluster(nodes, partitions);
        Map<Integer, List<Integer>> assignment = assignor.assignReplicasToBrokers("topic1", 1, 3, 0, cluster, principal);
        System.out.println(assignment);
        List<Integer> replicas = assignment.get(3);
        assertNotNull(replicas);
        Set<Integer> replicaSet = new HashSet<>(replicas);
        assertEquals(3, replicaSet.size());
    }
